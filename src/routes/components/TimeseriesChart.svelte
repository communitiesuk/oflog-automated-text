<script>
    export let dataForChart, timeValues, selectedPlace

    let groups = [...new Set(dataForChart.map(e=>e.group))]
    ...
    let drawLine = (dataForChart, 
                    group, 
                    line=dataForChart.filter(e=>e.group==group), 
                    firstVal=line.findIndex(e=>+e.value),
                    svg=`M${50 + (firstVal * (400/(timeValues.length-1)))} ${350-(((+line[firstVal].value - yMin) / yRange) * 300)} `) => {
                        line.forEach((e,i)=>+e.value && i>firstVal?
                        svg+=`L ${50+((400/(timeValues.length-1)) *i)} ${(350 - ((+e.value - yMin) / yRange) * 300)} `:"");
                        return svg;
                }

    console.log(drawLine(dataForChart,"median"))
